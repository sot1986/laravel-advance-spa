...
import type { ApiResponseI, ApiErrorsI } from '@/types/Api'
...
const getUser = async () => {
  ...
  if (prom.error.value) {
    console.log(prom.error.value.response.data.message)
    return false
  }
  ...
  console.log(prom.results.value)
...
const logout = async () => {
  const prom = usePromise(apiClient.post)
